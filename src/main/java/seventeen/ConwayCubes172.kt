...
import seventeen.ConwayCubes172.State.ACTIVE
import seventeen.ConwayCubes172.State.INACTIVE


class ConwayCubes172 {
    fun run() {
        ...
        var cubes =
            states.flatMapIndexed { x, line -> line.mapIndexed { y, state -> Coordinate(x, y, 0, 0) to state } }.toMap()
        ...
        for (x in 0 - turn..7 + turn) {
            for (y in 0 - turn..7 + turn) {
                for (z in 0 - turn..0 + turn) {
                    for (w in 0 - turn..0 + turn) {
                        result.add(Coordinate(x, y, z, w))
                    }
                }
            }
        }
        ...
        for (x in startCoordinate.x - 1..startCoordinate.x + 1) {
            for (y in startCoordinate.y - 1..startCoordinate.y + 1) {
                for (z in startCoordinate.z - 1..startCoordinate.z + 1) {
                    for (w in startCoordinate.w - 1..startCoordinate.w + 1) {
                        val coordinateKey = Coordinate(x, y, z, w)
